...
// The rx600-unknown-unknown target is currently an experimental version of a
// rx-based target which does *not* use the Emscripten toolchain. Instead
...
pub fn target() -> Result<Target, String> {
    let opts = TargetOptions {
        ...
        // final library artifact that exports some symbols (a rx module) but
        // we don't allow intermediate `dylib` crate types
        dynamic_linking: true,
        only_cdylib: true,

        // This means we'll just embed a `start` function in the rx module
        executables: true,

        // relatively self-explanatory!
        exe_suffix: ".rx600".to_string(),
        dll_prefix: String::new(),
        dll_suffix: ".rx600".to_string(),
        linker_is_gnu: false,
        ...
        // Rx600 doesn't have atomics yet, so tell LLVM that we're in a single
    ...
    Ok(Target {
        ...
        linker_flavor: LinkerFlavor::Lld(LldFlavor::Rx600),
        options: opts,
    })
}
